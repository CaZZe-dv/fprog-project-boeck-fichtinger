...
struct Node {
    ...
    Node(Color color, T val, NodePtr<T> left = nullptr, NodePtr<T> right = nullptr)
        : _color(color), _value(val), _left(left), _right(right) {}
};
...
NodePtr<T> rotateLeft(const NodePtr<T>& node) {
    ...
    return makeNode(newRoot->_color, newRoot->_value,
                    makeNode(Color::Red, node->_value, node->_left, newRoot->_left),
                    newRoot->_right);
}
...
NodePtr<T> rotateRight(const NodePtr<T>& node) {
    ...
    return makeNode(newRoot->_color, newRoot->_value,
                    newRoot->_left,
                    makeNode(Color::Red, node->_value, newRoot->_right, node->_right));
}
...
NodePtr<T> flipColors(const NodePtr<T>& node) {
    ...
    return makeNode(Color::Red, node->_value,
                    makeNode(Color::Black, node->_left->_value, node->_left->_left, node->_left->_right),
                    makeNode(Color::Black, node->_right->_value, node->_right->_left, node->_right->_right));
}
...
NodePtr<T> balance(Color color, NodePtr<T> left, const T& value, NodePtr<T> right) {
    if (color == Color::Black) {
        if (doubledLeft(left)) {
            return makeNode(Color::Red, left->_value,
                            makeNode(Color::Black, left->_left->_value, left->_left->_left, left->_left->_right),
                            makeNode(Color::Black, value, left->_right, right));
        }
        if (doubledRight(left)) {
            return makeNode(Color::Red, left->_right->_value,
                            makeNode(Color::Black, left->_value, left->_left, left->_right->_left),
                            makeNode(Color::Black, value, left->_right->_right, right));
        }
        if (doubledLeft(right)) {
            return makeNode(Color::Red, right->_left->_value,
                            makeNode(Color::Black, value, left, right->_left->_left),
                            makeNode(Color::Black, right->_value, right->_left->_right, right->_right));
        }
        if (doubledRight(right)) {
            return makeNode(Color::Red, right->_value,
                            makeNode(Color::Black, value, left, right->_left),
                            makeNode(Color::Black, right->_right->_value, right->_right->_left, right->_right->_right));
        }
    }
...
NodePtr<T> insert(const NodePtr<T>& node, const T& value) {
    if (!node)
        return makeNode(Color::Red, value);

    if (value < node->_value)
        return balance(node->_color, insert(node->_left, value), node->_value, node->_right);
    else if (value > node->_value)
        return balance(node->_color, node->_left, node->_value, insert(node->_right, value));
    else
        return node; //Returns the same node if the value is already present in the tree
}
...
private:
    ...
    bool validateNode(const NodePtr<T>& node, int& expectedBlackHeight, int currentBlackHeight) const {
        ...
        return validateNode(node->_left, expectedBlackHeight, currentBlackHeight) &&
               validateNode(node->_right, expectedBlackHeight, currentBlackHeight);
    }
};
